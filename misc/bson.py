...
if __name__ == "__main__": # self-test
    ...
    assert cstrify("àáâ") == b"\xd0\xb0\xd0\xb1\xd0\xb2\x00"
    ...
    test_type(BSON_String, "àáâ", b"\x07\x00\x00\x00\xd0\xb0\xd0\xb1\xd0\xb2\x00")
    ...
    test_type(BSON_JavaScript, "/* êîììåíòàðèé */",
    ...
    test_type(BSON_JavaScriptWithScope, ("foo(s);", BSON_Document({ "s": BSON_String("àáâ") })),
    ...
    b = serialize_to_bytes({ "string": "àáâ" })
    assert b == serialize_to_bytes({ "string": BSON_String("àáâ") })
    assert parse_bytes(b) == { "string": "àáâ" }
